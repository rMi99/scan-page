...
let Apps = ({qrData}) => {
  ...
  const t = useLanguage();
  useUpdateDomTitle(data?.app_title,data?.primaryColor);
  ...
  const imageH = Math.floor((imageHeight * 50)  / imageWidth);
  ...
  return welcomeScreen ? (
    ...
    <div 
      className="flex flex-col min-h-screen min-w-full items-center"
      // style={{background:data?.primaryColor && data?.SecondaryColor ? "linear-gradient(" +data?.primaryColor +"," +data?.SecondaryColor +")": "#000",}}
    >

    <svg className="fixed top-0 -z-20 w-full h-screen">
      <defs>
        ...
        >
          <stop stopColor={data?.primaryColor?data?.primaryColor:"#000"} className="duration-500" />
          <stop offset="1" stopColor={data?.SecondaryColor?data?.SecondaryColor:"#000"} className="duration-500" />
        </linearGradient>
      </defs>
      <rect className="h-full w-full" fill="url(#grad1)" />
    </svg>

      <div
        className="flex flex-col items-center justify-center mt-[50px] xs:mt-[60px] w-full max-w-[450px] px-[16px]"
      >
        {data?.app_title ? (
          <h1 
            className="text-lg xs:text-2xl font-bold text-center mb-[8px] xs:mb-[14px] duration-500"
            style={{color: light ? "#000000" : "#ffffff", fontFamily: data?.font_title}}
            id="color_palette"
          >
            {data?.app_title}
          </h1>
        ...
        {data?.app_company ? (
          <p 
            className="w-full mb-[10px] text-sm xs:text-base duration-500"
        ...
        <div className="flex flex-col items-center justify-center w-full bg-white pb-[26px] mb-[16px]  rounded-[10px] shadow-[1px_6px_30px_rgba(0,31,14,0.05)]" id="app_title">
          {data?.companyLogo ? (
            <>
              {data.companyLogo.includes(".png") || data.companyLogo.includes(".svg") ? (
                <img 
                  onLoad={handleImageLoad}
                  src={data.companyLogo} 
                  className={`w-[100px] mt-[-${imageH+"px"}] mb-[10px] xs:mb-[16px] rounded-[5px] shadow-[1px_6px_30px_rgba(0,31,14,0.05)]`}
                  style={{
                    marginTop: "-"+imageH+"px"
                  }}
                  alt=""
                ...
                <img 
                  onLoad={handleImageLoad}
                  src={data.companyLogo} 
                  className={`w-[95px] mt-[-${imageH+"px"}] mb-[10px] xs:mb-[20px] border-[1px] border-black rounded-[5px] shadow-[1px_6px_30px_rgba(0,31,14,0.05)]`}
                  style={{
                    marginTop: "-"+imageH+"px"
                  }}
                  alt=""
          ...
          {data?.app_description ? (
            <p className="w-full text-[#767C83] mb-[8px] xs:mb-[14px] px-[15px] text-[13px] xs:text-lg !leading-[1.35] whitespace-break-spaces text-balance" style={{fontFamily: data?.font_text, overflowWrap: "anywhere"}}>{data?.app_description}</p>
          ) : (
          ...
          <button 
            onClick={() => {
              if (data.app_website === "#"){
                ...
                openURL(data.app_website);
              }
            }}
            className="flex flex-row justify-between items-center bg-white w-full mb-[30px] p-[10px] rounded-[4px] cursor-pointer shadow-[1px_6px_30px_rgba(0,31,14,0.05)]"
            id="app_website"
          >
            <div className="space-x-[5px] xss:space-x-[15px] w-full mr-[5px] flex flex-row">
              ...
              <div className="flex items-center text-left truncate">
                <p className="text-sm xs:text-base sm:text-xl text-[#0079ff] truncate" style={{fontFamily: data?.font_text}}>{data.app_website === "#" ? "www.nourish.com" : <ShortenedUrl url={data.app_website} />}</p>
              </div>
            </div>
